...
void afficher_voisins() {
    ...
    for (int i = 0; i < nb_voisins; i++) {
        time_t diff = now - voisins[i].dernier_hello;
        if (diff <= VOISIN_TIMEOUT) {
            voisins[i].actif = 1;
            printf(" - %s (vu il y a %lds, capacite: %d Mbps) [ACTIF]\n",
                   voisins[i].ip, diff, voisins[i].capacite);
        } else {
            voisins[i].actif = 0;
            printf(" - %s (vu il y a %lds, capacite: %d Mbps) [INACTIF]\n",
                   voisins[i].ip, diff, voisins[i].capacite);
        }
    }
    printf("=====================\n\n");
...
void maj_voisin(const char* ip) {
    ...
    if (nb_voisins < MAX_VOISINS) {
        ...
        voisins[nb_voisins].actif = 1;
        voisins[nb_voisins].capacite = rand() % 90 + 10; // Simule une capacite entre 10 et 100 Mbps
        nb_voisins++;
        printf("[INFO] Nouveau voisin d\'etect\'e : %s\n", ip);
    }
}

void calculer_meilleurs_chemins() {
    printf("\n=== Meilleurs chemins ===\n");
    for (int i = 0; i < nb_voisins; i++) {
        if (!voisins[i].actif) continue;
        printf("Chemin vers %s : 1 saut, capacite: %d Mbps\n", voisins[i].ip, voisins[i].capacite);
    }
    printf("========================\n\n");
}
...
int main() {
    ...
    if (recup_interfaces() < 0) {
        fprintf(stderr, "Erreur : aucune interface r\'eseau active avec broadcast d\'etect\'ee\n");
        return EXIT_FAILURE;
    }
    ...
    printf("[INFO] Protocole HELLO d\'emarr\'e\n");
        ...
        if (now - last_hello >= HELLO_INTERVAL) {
            for (int i = 0; i < nb_interfaces; i++) {
                ...
                if (sent < 0) {
                    ...
                    printf("[DEBUG] HELLO envoy\'e depuis %s sur broadcast %s\n", interfaces[i].ip, interfaces[i].broadcast);
                }
            }
            last_hello = now;
            afficher_voisins();
            calculer_meilleurs_chemins();
        }
